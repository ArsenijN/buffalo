...
use obs_sys::{
    ...
    obs_source_frame, obs_source_frame_init, obs_source_get_base_height,
    ...
    obs_source_media_started, obs_source_media_stop, obs_source_output_video,
...
/// PATCHED IN LOCALLY -- not part of upstream obs-wrapper 0.4.1.
///
/// The published crate has no way to push frames into an async video (`OBS_SOURCE_ASYNC_VIDEO`)
/// input source; `obs_source_output_video`/`_video2` are only reachable through the raw
/// `obs-sys` bindings, and `SourceContext.inner` is private outside this module. This method
/// fills that gap for the one case buffalo needs: planar I420.
impl SourceContext {
    /// Pushes one planar I420 frame (`data` = Y plane, then U, then V, tightly packed with no
    /// row padding -- i.e. `width*height` bytes of Y followed by `(width/2)*(height/2)` bytes
    /// each of U and V) into this async video source.
    ///
    /// # Memory safety note
    /// `obs_source_frame_init` allocates the frame's plane buffers via OBS's own allocator
    /// (`bmalloc`), which is required since `obs_source_output_video` expects OBS-managed
    /// memory. Per libobs's documented contract, `obs_source_output_video` copies the frame
    /// data internally before it returns, so freeing our buffer with `bfree` immediately after
    /// the call is safe and required (otherwise this leaks one frame's worth of memory on
    /// every call). This assumes `obs_source_frame_init` allocates `data[0]` as a single block
    /// that the other plane pointers are offset into -- true for every libobs version this was
    /// checked against, but if you see growing memory use over a long stream, that assumption
    /// is the first thing to re-check against your installed libobs's obs-source.c.
    pub fn output_video_i420(&mut self, data: &[u8], width: u32, height: u32, timestamp_ns: u64) {
        unsafe {
            ...
            let u_size = (frame.linesize[1] as usize) * (chroma_h as usize);
            let v_size = (frame.linesize[2] as usize) * (chroma_h as usize);
            ...
            {
                std::ptr::copy_nonoverlapping(data.as_ptr(), frame.data[0], y_size);
                std::ptr::copy_nonoverlapping(data.as_ptr().add(y_size), frame.data[1], u_size);
                std::ptr::copy_nonoverlapping(
                    data.as_ptr().add(y_size + u_size),
                    frame.data[2],
                    v_size,
                );

                frame.timestamp = timestamp_ns;
                obs_source_output_video(self.inner, &frame);
            }

            if !frame.data[0].is_null() {
                bfree(frame.data[0] as *mut std::ffi::c_void);
            }
        }
    }
}
